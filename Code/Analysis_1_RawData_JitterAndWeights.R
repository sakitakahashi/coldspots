...
for(i in 1:length(codes$Country)) {
	...
	# Jitter urban clusters up to 2 km, and jitter rural clusters up to 5 km
	jittering <- matrix(NA, nrow(dataClust), 3)
	for(j in 1:nrow(dataClust)) jittering[j,] <- lat_long_distance_jitter(lat1=dataClust$lat[j], long1=dataClust$long[j], urban_rural=dataClust$urban_rural[j])
	colnames(jittering) <- c("long_jittered", "lat_jittered", "dist_obs_jittered")
	dataClust <- cbind(dataClust, jittering)
	
	# print(codes$DHS_code[which(codes$Country==codes$Country[i])])
	# print(paste0("Max. Urban: ", round(max(dataClust$dist_obs_jittered[which(dataClust$urban_rural=="U")], na.rm=T),3)))
	# print(paste0("Max. Rural: ", round(max(dataClust$dist_obs_jittered[which(dataClust$urban_rural=="R")], na.rm=T),3)))
	
	# Append the jittered points
	dataInd_country$long_jittered <- dataClust$long_jittered[match(dataInd_country$cluster_id,dataClust$cluster_id)]	
	dataInd_country$lat_jittered <- dataClust$lat_jittered[match(dataInd_country$cluster_id,dataClust$cluster_id)]	
	dataInd_country$dist_obs_jittered <- dataClust$dist_obs_jittered[match(dataInd_country$cluster_id,dataClust$cluster_id)]			
	...
	# Save the data
	save(all_SIAs, which_SIAs_subnational, dataInd_country_subnational, dataInd_country, dataSIA_country, file=paste0("./Data/Manipulated_data_for_GAM_JitterAndWeights/", codes$DHS_code[which(codes$Country==codes$Country[i])], ".RData"))
